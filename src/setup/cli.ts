...
import type { Adapter, FileEdit, Probe, SetupContext } from "./types.js";

type Command = "setup" | "uninstall" | "status" | "help" | "version";
...
async function main(argv: string[]): Promise<number> {
  ...
  const ctx: SetupContext = { catalog, home: homedir(), cwd: process.cwd() };

  const probe: Probe = { exists: existsSync, hasBinary };
  ...
  if (selected.length === 0) {
    process.stdout.write(
      `${yellow("No supported client detected.")}\n` +
  ...
  const plans: Array<{ adapter: Adapter; edits: FileEdit[] }> = [];
  for (const adapter of selected) {
    const edits = options.command === "uninstall" ? adapter.uninstall(ctx, read) : adapter.install(ctx, read);
    if (edits.length > 0) plans.push({ adapter, edits });
  }

  const writable = plans.filter(({ edits }) => edits.some((edit) => !edit.blocked));
  const blocked = plans.flatMap(({ adapter, edits }) =>
    edits.filter((edit) => edit.blocked).map((edit) => ({ adapter, edit })),
  );
  ...
  if (options.command === "setup") printNextSteps(ctx, writable.map(({ adapter }) => adapter), apiKey);
  return 0;
}

function selectAdapters(options: Options, ctx: SetupContext, probe: Probe): Adapter[] {
  if (options.clients.length > 0) {
    return options.clients.map((id) => {
      const adapter = findAdapter(id);
      if (!adapter) throw new Error(`Unknown client "${id}". Known: ${ADAPTER_IDS.join(", ")}`);
      return adapter;
    });
  }
  if (options.all) return [...ADAPTERS];
  return ADAPTERS.filter((adapter) => adapter.detect(ctx, probe));
}

function printCatalogBanner(ctx: SetupContext): void {
  const { catalog } = ctx;
  const source =
    catalog.source === "live"
  ...
  process.stdout.write(
    `${bold("GrowthCircle.id")}  tier=${catalog.tier}  models=${catalog.modelIds.length}  ${source}\n` +
      `default model: ${catalog.defaultModelId}\n\n`,
  );
}

function printEdit(adapter: Adapter, edit: FileEdit): void {
  ...
  for (const line of edit.summary) process.stdout.write(`  · ${line}\n`);
...
function applyEdit(edit: FileEdit): void {
  if (edit.before !== null) {
...
function printNextSteps(ctx: SetupContext, adapters: Adapter[], apiKey: string | undefined): void {
  process.stdout.write(`\n${bold("Set your key")}\n`);
  if (apiKey) {
  ...
  for (const adapter of adapters) {
    ...
    for (const line of adapter.usage(ctx)) process.stdout.write(`    ${line}\n`);
  }
}

function printStatus(ctx: SetupContext, probe: Probe): void {
  printCatalogBanner(ctx);
...
function parseArgs(argv: string[]): Options {
  ...
  for (; index < argv.length; index += 1) {
    ...
    else if (arg === "--help" || arg === "-h") return { ...options, command: "help" };
    else if (arg === "--version" || arg === "-V") return { ...options, command: "version" };
    else if (arg === "--tier") {
      const value = argv[index + 1];
      if (!value || !isTier(value)) throw new Error(`--tier expects one of: ${TIERS.join(", ")}`);
      options.tier = value;
      index += 1;
    } else if (arg.startsWith("--tier=")) {
      const value = arg.slice("--tier=".length);
      if (!isTier(value)) throw new Error(`--tier expects one of: ${TIERS.join(", ")}`);
      options.tier = value;
...
function isTier(value: string): value is GrowthCircleKeyTier {
  ...
  for (let depth = 0; depth < 6; depth += 1) {
    ...
    if (existsSync(candidate)) {
      try {
        ...
        if (pkg.name === "gc-provider" && pkg.version) return pkg.version;
      } catch {
        // keep walking up
      }
    }
...
function colorizeDiff(line: string): string {
  ...
  if (line.trimStart().startsWith("…")) return dim(line);
  return line;
}
