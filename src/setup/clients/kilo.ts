 ...
 * unresolved.
 */
export function kiloConfigPath(ctx: SetupContext): string {
  const xdg = process.env.XDG_CONFIG_HOME?.trim();
  return join(xdg && xdg.length > 0 ? xdg : join(ctx.home, ".config"), "kilo", "kilo.json");
}

function buildModels(ctx: SetupContext): Record<string, unknown> {
...
export const kiloAdapter: Adapter = {
  ...
  detect: (ctx, probe) =>
    probe.exists(join(ctx.home, ".config", "kilo")) || probe.hasBinary("kilo"),
  ...
  uninstall: (ctx, read): FileEdit[] => {
    const path = kiloConfigPath(ctx);
    const before = read(path);
    if (before === null || isUnparseableJsonc(before)) return [];
    const base = ["provider", PROVIDER_ID];
    if (getJsoncIn(before, base) === undefined) return [];
