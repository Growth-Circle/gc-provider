...
export function zedConfigDir(ctx: SetupContext): string {
  const xdg = process.env.XDG_CONFIG_HOME?.trim();
  return join(xdg && xdg.length > 0 ? xdg : join(ctx.home, ".config"), "zed");
}

export function zedSettingsPath(ctx: SetupContext): string {
  return join(zedConfigDir(ctx), "settings.json");
}

function buildAvailableModels(ctx: SetupContext) {
