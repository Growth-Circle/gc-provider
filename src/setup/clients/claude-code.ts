...
const KEYS = ["ANTHROPIC_BASE_URL", "ANTHROPIC_MODEL", "ANTHROPIC_SMALL_FAST_MODEL"] as const;

export function claudeSettingsPath(ctx: SetupContext): string {
...
export const claudeCodeAdapter: Adapter = {
  ...
  uninstall: (ctx, read): FileEdit[] => {
    ...
    for (const key of KEYS) {
      if (getJsoncIn(text, ["env", key]) === undefined) continue;
