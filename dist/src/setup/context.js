import { DEFAULT_FREE_MODEL_ID, DEFAULT_MODEL_ID, FREE_TEXT_MODEL_IDS_WITH_SUFFIX, PAID_TEXT_MODEL_IDS, TEAM_TEXT_MODEL_IDS, fetchGrowthCircleModels, isGrowthCircleApiKeyForTier, } from "../provider.js";
const TIERS = ["free", "paid", "team"];
/** Reads the tier from the key prefix (gc-free-/gc-paid-/gc-team-). */
export function resolveGrowthCircleTier(apiKey) {
    if (!apiKey)
        return null;
    return TIERS.find((tier) => isGrowthCircleApiKeyForTier(apiKey, tier)) ?? null;
}
export function staticModelIdsForTier(tier) {
    if (tier === "free")
...
export function defaultModelIdForTier(tier) {
 ...
 */
export function pickDefaultModelId(modelIds, tier) {
...
function newestMatching(modelIds, keyword) {
 ...
 */
export function pickAnthropicModelId(modelIds) {
...
export function pickAnthropicSmallModelId(modelIds) {
    const pool = claudePool(modelIds);
    if (pool.length === 0)
        return null;
...
export async function resolveCatalog(options = {}) {
    ...
    const staticCatalog = (reason) => buildCatalogFromModelIds({
    ...
    if (!apiKey)
        return staticCatalog(`${"GROWTHCIRCLE_API_KEY"} is not set`);
    if (options.offline)
        return staticCatalog("--offline was requested");
    try {
        const fetchModels = options.fetchModels ?? fetchGrowthCircleModels;
        const models = await fetchModels({ apiKey });
        const modelIds = models.map((model) => model.id);
        if (modelIds.length === 0)
            return staticCatalog("GrowthCircle.id returned an empty model list");
